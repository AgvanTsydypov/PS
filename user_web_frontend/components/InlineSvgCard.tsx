...
export function InlineSvgCard({ url, className, style, alt }: InlineSvgCardProps) {
  ...
  useEffect(() => {
    ...
    if (_fetchCache.has(url)) {
      ...
      const safe = DOMPurify.sanitize(cached, CARD_SVG_PURIFY_CONFIG);
      if (!safe.trim() || !safe.includes("<svg")) return;
      setSvg(scopeIds(safe, prefix));
      return;
    }
    ...
    fetch(`/api/svg-proxy?url=${encodeURIComponent(url)}`, {
      ...
      .then((text) => {
        ...
        const safe = DOMPurify.sanitize(patched, CARD_SVG_PURIFY_CONFIG);
        if (!safe.trim() || !safe.includes("<svg")) {
          return;
        }
        _fetchCache.set(url, safe);
        setSvg(scopeIds(safe, prefix));
      })
      .catch((err) => {
  ...
  return (
    <div
      ...
      style={style}
      // SVG is fetched from allowlisted origins, sanitized (DOMPurify), then ID-scoped
