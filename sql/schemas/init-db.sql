...
-- ============================================================================
-- MINT RUNTIME SETTINGS (admin chooses gas tier for cron worker mint broadcast)
-- ============================================================================
-- Singleton row holding the currently selected gas tier ('safe' | 'propose' |
-- 'rapid'). Read by daily_scheduler_simple.process_mint_queue on every batch:
-- it picks GasTrackerSnapshot.<tier>_gwei for the on-chain broadcast and
-- <tier>_usd for the price-gate threshold comparison. Mutated by the admin
-- via PUT /api/mint-settings/speed-tier so changes apply to the very next
-- claim picked off the queue without restarting any container.
...
DO $$ BEGIN RAISE NOTICE '✅ polystars_mint_runtime_settings ready'; END $$;
